...
class EmployeeController extends Controller
{
    public function empCreateForm(){
        ...
        $roles = Role::all();
        //$users = User::all();
        $users = User::join('departments', 'users.id', '=', 'departments.departmentHead')
                ->distinct()
                ->get(['users.*']);
        //return $users;
        return view('Hrm.Employee.addEmp')->with('departments',$deparments)->with('designations',$designation)->with('roles',$roles)->with('users',$users);
    ...
    public function empCreate(Request $request){
        ...
        if($userCheck){
            ...
            $user->headOfDepartmentId = $request->dhead;
            ...
            if($request->password === $request->cpassword){
                ...
                return  redirect()->route('empList')->with('Create_Message', 'Password and confirm password doesnt match');
            }
    ...
    public function empList(){
        ...
        $designation = Designation::all();
        $roles = Role::all();
        $heads = User::join('departments', 'users.id', '=', 'departments.departmentHead')
                ->distinct()
                ->get(['users.*']);

        return view('Hrm.Employee.empList')->with('users',$users)->with('departments',$dept)->with('designations',$designation)->with('roles',$roles)->with('heads',$heads);
    }
    ...
    public function getEmployee(Request $request){
        ...
        ->distinct()
        ->get(['users.*']);
        $headName = User::where('headOfDepartmentId',$user->headOfDepartmentId)->first();
        return view('Hrm.Employee.basicInfo')->with('user',$user)->with('departments',$departments)->with('departmentName',$departmentName)->with('designations',$designations)->with('designationName',$designationName)->with('roles',$roles)->with('roleName',$roleName)->with('heads',$heads)->with('headName',$headName);
    }
